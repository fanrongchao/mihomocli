...
When adding merge-time conveniences, prefer CLI flags. The existing `--dev-rules` option prepends proxy rules for GitHub/Docker/GCR/cache.nixos.org and should remain stable (adjust targets via `--dev-rules-via`).
